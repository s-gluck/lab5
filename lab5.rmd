...
library(rio)
library(tidyverse)
#library(ggplot2) minor - but this is loaded when you load the tidyverse
library(here)
library(scales)
library(janitor)
library(ggrepel)
...
p1_d <- d %>% 
  select(state, year, ends_with("revenue"), -total_revenue) %>% 
  pivot_longer(ends_with("revenue"), 
...
p1_d %>% 
  mutate(revenue_type = str_replace(revenue_type, "_revenue", "")) %>% # remove _revenue
...
The legend with all the states in different colors makes the plot extremely diffuclt to understand and the colors that correspond to each state is difficult to distinguish. Instead of having all the states by different colors, a stronger visualization could have all the states in one color and selectively highlight particular states of interest as it comes to revenue (e.g., states with highest and lowest federal, local and state revenue, or highlist the state that one resides in) or the addition of a trend line to show the average change in revenue over time. 
...
ggplot(p1_d_revised) +
  ...
  gghighlight(revenue == min(revenue) | revenue == max(revenue), 
              calculate_per_facet = TRUE, max_highlight = 1) + 
  theme_minimal()

# huh... the above doesn't really work at all for me.
...
# 🤷 I'm not exactly sure what's going on. Seems like gghighlight is just 
# having a hard time here. Maybe just approach it manually.
```
...
<!-- I modified your regular expression so hopefully it makes a bit more sense.  -->
<!-- The `\\1` is saying give me the first parens. We only have one in this case -->
<!-- but in your case you had multiple and you were asking for the second. -->
<!-- The `.+` means any character multiple as many times as needed and  -->
<!-- the \\d means a digit. -->

```{r p2_data_manip}
p2_d <- d %>% 
  select(state, year, total_revenue, enroll, ends_with("score")) %>% 
  drop_na(-state, -year) %>%  
  pivot_longer(cols = ends_with("score"), 
               names_to = "content",
               values_to = "score") %>% 
  mutate(content = gsub("avg_(.+)_\\d.+", "\\1", content)) %>% 
...
# I love this so much! Great job using the text annotations and ggtext!
